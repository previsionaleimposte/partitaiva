...
function preload() {
  ...
  font = loadFont('assets/GaisyrMono-Regular.otf');
}
...
function resetSketch() {
  ...
  arrowSize = myWidth * arrowProportion;
  noTileValX = floor(((myWidth - arrowSize)/2)/tileW)-2;
  noTileValY = floor(((myHeight - arrowSize)/2)/tileH)-2;
  mosaicUpdateCycles = 0;
  arrowRotationUpdateCycles = 0;
  ...
  //viene creato un array che contiene i valori univoci, (poi shuffolato volendo, ma senza le celle hanno numeri consequenziali)
  ...
  for (let i = noTileValX; i < numTilesW-noTileValX; i++) {
    //array temporaneo che contiene le tiles della riga i
    let arr = [];

    for (let j = noTileValY; j < numTilesH-noTileValY; j++) {
...
function draw() {
  ...
  if (arrowRotationUpdateCycles == 0) {
  ...
  if (fpsArrow / refreshMosaic == fpsMosaic){
    ...
    for (let i = noTileValX; i < numTilesW - noTileValX; i++){
      for (let j = noTileValY; j < numTilesH - noTileValY; j++) {
        ...
        //se serve aggiornare le lettere o se hanno già raggiunto la lettera finale
        if (mosaicUpdateCycles <= maxIter) {
  ...
  //bottom
  for (let i = 0; i < numTilesW; i++){
    for (let j = numTilesH-noTileValY; j < numTilesH; j++) {
      push();
      fill(255);
      //noStroke();
      rect(i * tileW, j * tileH, tileW, tileH);
  ...
  //right
  for (let i = numTilesW-noTileValX; i < numTilesW; i++){
    for (let j = noTileValY; j < numTilesH-noTileValY; j++) {
      push();
      fill(255);
      //noStroke();
